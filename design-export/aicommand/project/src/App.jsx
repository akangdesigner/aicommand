  ...
  return (
    <header className="sticky top-0 z-20 border-b border-stone-200/80 bg-[#FBFBF9]/85 backdrop-blur">
      <div className="mx-auto flex h-14 max-w-[1180px] items-center justify-between gap-4 px-5 sm:px-8">
        <button onClick={onLogo} className="flex items-center gap-2.5 text-stone-900">
          <div className="relative h-6 w-6">
            <div
              className="absolute inset-0 rounded-md"
              style={{
                background:
                  'linear-gradient(135deg, oklch(0.55 0.18 265) 0%, oklch(0.6 0.16 200) 100%)',
              }}
            />
        ...
        <nav className="hidden items-center gap-1 text-[13px] text-stone-600 lg:flex">
          <a className="rounded-full px-3 py-1.5 hover:bg-stone-100" href="#ranking">排行榜</a>
...
function App() {
  const [route, setRoute] = useStateApp(() => {
    ...
    if (h.startsWith('tools/')) return { name: 'detail', slug: h.split('/')[1] };
    return { name: 'home' };
  });

  useEffectApp(() => {
    if (route.name === 'detail') {
      window.location.hash = `tools/${route.slug}`;
  ...
  useEffectApp(() => {
    const onHash = () => {
      const h = window.location.hash.slice(1);
      if (h.startsWith('tools/')) setRoute({ name: 'detail', slug: h.split('/')[1] });
  ...
  return (
    <div className="min-h-screen bg-[#FBFBF9] text-stone-900">
      <TopNav onLogo={() => setRoute({ name: 'home' })} />
      {route.name === 'home' ? (
        <div data-screen-label="01 排行榜首頁">
          <HomePage onOpen={(slug) => setRoute({ name: 'detail', slug })} />
        </div>
      ) : (
        <div data-screen-label={`02 工具詳情 — ${route.slug}`}>
          <DetailPage
            slug={route.slug}
            onBack={() => setRoute({ name: 'home' })}
            onOpen={(slug) => setRoute({ name: 'detail', slug })}
          />
        </div>
      )}
    </div>

          ...
          >
            ...
            <span
              className={cx(
                ...
                active ? 'text-white/60' : 'text-stone-400 group-hover:text-stone-500'
              )}
            >
              {counts[c] ?? 0}
            </span>
        ...
        return (
          <button
            ...
            className={cx(
              ...
              active ? 'bg-stone-100 text-stone-900' : 'text-stone-500 hover:text-stone-800'
            )}
          >
            {s.label}
          </button>
...
function CompactRow({ tool, onOpen }) {
  return (
    <button
      onClick={() => onOpen(tool.slug)}
...
function HomePage({ onOpen }) {
  const [cat, setCat] = useStateHome('全部');
  const [sort, setSort] = useStateHome('hot');

  const counts = useMemoHome(() => {
    const c = { '全部': window.TOOLS.length };
    window.CATEGORIES.slice(1).forEach((k) => {
      c[k] = window.TOOLS.filter((t) => t.category === k).length;
    });
    return c;
  }, []);

  const filtered = useMemoHome(() => {
    let arr = cat === '全部' ? [...window.TOOLS] : window.TOOLS.filter((t) => t.category === cat);
    ...
    <div className="mx-auto max-w-[1180px] px-5 pb-24 pt-10 sm:px-8">
      ...
      <div className="sticky top-[57px] z-10 -mx-5 mb-5 border-y border-stone-200/80 bg-[#FBFBF9]/85 px-5 py-3 backdrop-blur sm:-mx-8 sm:px-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <FilterTabs value={cat} onChange={setCat} counts={counts} />
      ...
      {featured.length === 3 && (
        <section className="mb-6 grid grid-cols-1 gap-4 pt-3 md:grid-cols-3">
          {featured.map((t) => (
            <FeaturedCard key={t.slug} tool={t} onOpen={onOpen} />
          ))}
        </section>
      )}
      ...
      <section className="overflow-hidden rounded-2xl border border-stone-200 bg-white">
        ...
        {rest.length === 0 ? (
          <div className="px-6 py-10 text-center text-[13px] text-stone-500">
            此分類目前還沒有更多工具
          </div>
        ) : (
          rest.map((t) => <CompactRow key={t.slug} tool={t} onOpen={onOpen} />)
        )}
      </section>

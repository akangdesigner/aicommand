// Detail page: per-tool insights
const { useMemo: useMemoDetail } = React;
            ...
            <>
              ...
              <g transform={`translate(${p[0] + 10}, ${p[1] - 14})`}>
                ...
                <text
                  ...
                  textAnchor="middle"
                  fill="white"
                  style={{ fontFamily: 'Geist Mono, monospace', fontSize: 11, fontWeight: 500 }}
                >
                  {data[i].toFixed(1)}
                </text>
              </g>
...
function InsightCard({ title, hint, children, accent }) {
  return (
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span
            ...
            style={{ background: accent || 'oklch(0.55 0.18 265)' }}
          />
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {items.map((t) => (
        <span
          key={t}
          className="inline-flex items-center rounded-full border border-stone-200 bg-stone-50 px-2.5 py-1 text-[12.5px] text-stone-700"
        >
          {t}
        </span>
      ))}
...
function BulletList({ items, marker = 'oklch(0.55 0.18 265)' }) {
  return (
...
function Quote({ q }) {
  ...
  return (
    <figure className="relative rounded-xl border border-stone-200 bg-white p-5">
      <div className="absolute left-5 top-3 text-[40px] leading-none text-stone-200 font-serif select-none">
        “
      </div>
      ...
      <figcaption className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1.5 pl-7 text-[12px] text-stone-500">
        ...
        <span
          ...
          style={{ background: sentimentStyle.bg, color: sentimentStyle.fg }}
        >
...
function DetailPage({ slug, onBack, onOpen }) {
  const tool = useMemoDetail(() => window.TOOLS.find((t) => t.slug === slug), [slug]);
  if (!tool) return null;
    ...
    <div className="mx-auto max-w-[1180px] px-5 pb-24 pt-6 sm:px-8">
        ...
        <div>
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-3 max-w-[520px] text-[15px] leading-snug text-stone-600">
                {tool.description}
              </p>
            </div>
          </div>
              ...
              <div className="mt-1 flex items-baseline gap-2">
                ...
                {rankDelta !== 0 && (
                  <span
                    ...
                    style={{
                      color:
                        rankDelta > 0 ? 'oklch(0.5 0.16 145)' : 'oklch(0.55 0.18 25)',
                    }}
                  >
                    {rankDelta > 0
                      ? `↑${rankDelta} 自 #${tool.prevRank}`
                      : `↓${Math.abs(rankDelta)} 自 #${tool.prevRank}`}
                  </span>
                )}
              </div>
            </div>
      ...
      <section className="mb-8">
        <div className="mb-4 flex items-baseline justify-between gap-3">
          ...
          <span className="text-[11px] tracking-[0.04em] text-stone-400">
            整合自 {tool.discussions.toLocaleString()} 則討論
          </span>
        </div>
      ...
      <section className="mb-8 grid grid-cols-1 gap-3 lg:grid-cols-[1.5fr_1fr]">
        <div className="rounded-2xl border border-stone-200 bg-white p-5">
          <div className="mb-4 flex items-baseline justify-between gap-3">
            <div>
              <h3 className="text-[13px] font-semibold tracking-[-0.01em] text-stone-900">
                熱度趨勢 · 近 8 週
              </h3>
              <p className="mt-0.5 text-[11.5px] text-stone-500">
                每日熱度,以週為單位平滑。數字越高代表社群正面聲量越多。
              </p>
            </div>
        ...
        <div className="space-y-3">
          <div className="rounded-2xl border border-stone-200 bg-white p-5">
            <h3 className="mb-3 text-[13px] font-semibold tracking-[-0.01em] text-stone-900">
              常被拿來比較
            </h3>
            <div className="space-y-2.5">
              {tool.competitors.map((c) => {
                const t = window.TOOLS.find((x) => x.name === c.name);
                return (
                  <button
                    key={c.name}
                    onClick={() => t && onOpen(t.slug)}
                    disabled={!t}
                    className={cx(
                      'flex w-full items-center gap-3 rounded-lg border border-stone-100 px-3 py-2 text-left transition',
                      t ? 'hover:border-stone-200 hover:bg-stone-50' : 'opacity-70'
                    )}
                  >
                    {t ? (
                    ...
                    <span className="font-mono text-[12.5px] tabular-nums text-stone-600">
                      {c.count} 次
                    </span>
                  </button>
                );
      ...
      <footer className="text-[12px] text-stone-400">
        <span className="text-stone-600 font-medium">{tool.name}</span> 上次更新於 14 分鐘前 ·
        下次更新還有 46 分鐘。
      </footer>
    </div>
  );
